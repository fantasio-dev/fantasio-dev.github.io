...
이 페이지는 **기출문제 템플릿이 아니라**, “AI를 한 장으로 연결해서 이해하는” **개인 학습 가이드**입니다.  
아래에 제가 보기 좋은 구조로 재정리한 **가이드 뷰**가 있고, 맨 아래에 **원문(CSV) 전체를 그대로** 넣었습니다.  
→ 그래서 **원문 글자가 1글자도 빠지지 않습니다.**

---

## 어떻게 읽으면 좋을까? (제안)

- **1) 큰 흐름 먼저**: `수저처 → 선학평 → 배모튜` (데이터→모델→운영)
- **2) 학습 루프**: `인추판제행학` + 피드백
- **3) 모델 선정은 “지도/비지도/강화 + 딥러닝/생성형/멀티모달/XAI/자율형”으로 분류**
- **4) 평가는 체크리스트로 통째로 외우기**
- **5) 도구는 “단계별로 어떤 툴을 쓰는지”만 맵핑**
...
{: .highlight }
> **AI 개발 Lifecycle / 머신러닝 파이프라인**: 데이터 → 모델 → 운영(피드백)
> - (데이터) `수저처` 수집·저장·전처리
> - (모델) `선학평` 선정·학습·평가
> - (운영) `배모튜` 배포·모니터링·튜닝
>
> **학습 루프**: `인추판제행학` 인식→추론→판단→제어→행동→학습 (+ Feedback Loop)
...
| 구분 | 키워드 | 설명 |
|:--|:--|:--|
| 데이터 분리 | Training / Validation / Test set | 일반화 성능 확인 |
| 검증 방법 | Hold-out / K-Fold Cross Validation / LOOCV | 반복 평가 |
| 평가 지표 | Accuracy, Precision, Recall, F1-score, RMSE 등 | 수치화 |
| 과적합 확인 | Overfitting / Underfitting | 학습 편향 방지 |
| 베이스라인 모델 | Baseline model | 개선 효과 판단 |
| 하이퍼파라미터 튜닝 | Grid / Random / Bayesian | 성능 최적화 |
| 모델 비교 | Model Selection | 최적 모델 선택 |
| 신뢰성 검증 | Robustness / Fairness / Bias | 공정/안정 |
...
| 단계 | 세부 단계 | 주요 도구(원문 발췌) | 설명 |
|:--|:--|:--|:--|
| 데이터 | 데이터 수집 | Airflow, Kafka, Flume, AWS Glue, Fivetran, Scrapy, BeautifulSoup | 크롤링/배치/스트리밍 수집 |
|  | 데이터 저장 | S3, HDFS, BigQuery, Snowflake, PostgreSQL, MongoDB, Delta Lake, Iceberg | Data Lake/Warehouse |
|  | 전처리 | Spark, Pandas, DBT, Databricks, Trino, Feature Store (Feast) | 정제/피처/통합 |
| 모델 | 모델 선정 | Scikit-learn, AutoML, DataRobot, Optuna | 탐색/튜닝 |
|  | 모델 학습 | TensorFlow, PyTorch, Keras, XGBoost, LightGBM, Hugging Face Transformers, Kubeflow Pipelines | 학습/분산 |
|  | 모델 평가 | MLflow, W&B, TensorBoard, Neptune.ai, Comet.ml | 실험/버전 |
| 운영 | 모델 배포 | Docker, Kubernetes, Seldon Core, TensorFlow Serving, FastAPI, BentoML, SageMaker | 서빙/배포 |
|  | 모니터링 | Prometheus, Grafana, Evidently AI, WhyLabs, Arize AI | 드리프트/성능 |
|  | 튜닝 | Optuna, Ray Tune, MLflow Tracking, Hyperopt, Katib (Kubeflow) | 재학습/개선 |
...
<details markdown="1">
<summary><strong>AI - MAP.csv 원문 전체</strong> (클릭해서 펼치기)</summary>
