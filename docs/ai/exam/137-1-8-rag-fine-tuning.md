...
| 개념 | 정의 | 쉬운 비유 |
|:-----|:-----|:---------|
| **RAG** | AI가 답하기 전에 외부 문서를 검색해서 참고하는 기술 | 오픈북 시험 - 책을 보면서 답변 작성 |
| **Fine-Tuning** | AI 모델을 특정 분야 데이터로 재학습시키는 기술 | 전문가 양성 과정 - 특정 분야를 깊이 공부시킴 |
...
| 키워드 | 설명 | 예시 |
|:------|:-----|:-----|
| **검색 증강 생성** | 외부 지식베이스를 검색하여 프롬프트에 증강, LLM 정확성과 신뢰성 향상 | LangChain, LlamaIndex |
| **파라미터 조정** | 사전학습된 모델의 가중치를 특정 도메인 데이터셋으로 재학습 | LoRA, PEFT |
| **할루시네이션** | LLM이 사실이 아닌 정보를 생성하는 현상, RAG로 완화 가능 | 사실 검증 필요 |
...
| 구분 | 내용 |
|:-----|:-----|
| **기술환경 변화** | LLM은 학습 시점 이후 정보를 알지 못하고, 특정 도메인 지식이 부족 |
| **비즈니스 요구** | 할루시네이션 해결, 최신 정보 반영, 도메인 전문성 강화 필요 |
...
| 암기 | 단계 | 절차 | 설명 | 예시 |
|:-----|:-----|:-----|:-----|:-----|
| **질** | 검색 | 질문 임베딩 | 자연어 질문을 수치적 벡터로 표현 | Sentence Embedding |
| **유** | 검색 | 유사성 검색 | Vector DB에서 질문과 데이터 문서의 유사성 검색 | Cosine Similarity |
| **관** | 검색 | 관련도 순위 결정 | 가장 관련성 높은 데이터 순위 재조정, 메타데이터 필터링 | ReRank 모델 |
| **추** | 생성 | 추론 | Vector DB에서 관련 문서 조각을 가져와 최종 프롬프트 구성 | Top-K 문서 |
| **프** | 생성 | 프롬프트 엔지니어링 | 검색된 정보 기반 사용자 질문에 적절히 답변하도록 구성 | 시스템 프롬프트 |
| **텍** | 생성 | 텍스트 응답 생성 | 생성된 텍스트가 문법적으로 정확하고 일관성 있는지 확인 | 후처리 |

{: .note }
> 🧠 암기법: **"질유관추프텍"** (RAG 절차)
> 
> `질문임베딩 → 유사성검색 → 관련도순위 → 추론 → 프롬프트 → 텍스트생성` (검색 → 생성 흐름)

#### 그룹 2: Fine-Tuning 절차 `데모하파평`

| 암기 | 단계 | 절차 | 설명 | 예시 |
|:-----|:-----|:-----|:-----|:-----|
| **데** | 사전 | 데이터셋 준비 | 추가 학습 데이터셋 수집, 저장 및 전처리 수행 | 법률 문서, 의료 기록 |
| **모** | 사전 | 모델 선택 | 데이터셋 특성 기반 사전 학습모델 선택 | Llama, GPT-3.5 |
| **하** | 튜닝 | 하이퍼파라미터 설정 | 학습률, 배치 크기, 훈련 에포크 수 결정 | Grid Search, Bayesian |
| **파** | 튜닝 | 파인튜닝 학습 | 지도/비지도 파인튜닝, 전이학습, 미세조정 | LoRA, PEFT |
| **평** | 튜닝 | 평가/튜닝 | Accuracy, F1-score, BLEU, ROUGE 등 성능 평가 | BERTScore |

{: .note }
> 🧠 암기법: **"데모하파평"** (Fine-Tuning 절차)
> 
> `데이터셋 → 모델선택 → 하이퍼파라미터 → 파인튜닝 → 평가` (준비 → 학습 → 검증)

---

### 📊 RAG vs Fine-Tuning 비교 `개특장단활프`

| 암기 | 구분 | RAG | Fine-Tuning |
|:-----|:-----|:----|:------------|
| **개** | 개념 | 외부 지식베이스 **검색 후 프롬프트에 증강**하여 최신/도메인 지식 반영 | 특정 도메인 데이터셋으로 **모델 파라미터 직접 조정** |
| **특** | 특징 | 모델 변경 없음 (Zero-shot/Few-shot), 최신 데이터 반영 가능, 데이터 업데이트 용이 | 모델 파라미터 직접 조정, 특화 모델 생성, 데이터 변경 시 재학습 필요 |
| **장** | 장점 | 최신 데이터 반영, 비용 효율적, 도메인 지식 관리 용이 | 응답 일관성 ↑, 특정 스타일/전문성 강화, 오프라인 동작 |
| **단** | 단점 | 검색 품질에 따라 성능 좌우, 검색 인덱스 관리 필요 | 재학습 비용 큼 (시간·GPU), 오버피팅 가능성 |
| **활** | 활용 | 챗봇, 기업 FAQ, 실시간 문서 검색 기반 질의응답 | 법률 AI, 의료 AI, 고객센터 전용 챗봇, 특정 언어/스타일 모델 |
| **프** | 프레임워크 | LangChain, LlamaIndex, Haystack, RAGFlow | PEFT, Unsloth, InstructLab, OpenAI API |

{: .note }
> 🧠 암기법: **"개특장단활프"** (비교 항목)
> 
> `개념 → 특징 → 장점 → 단점 → 활용 → 프레임워크`
...
| 암기 | 프레임워크 | 설명 |
|:-----|:----------|:-----|
| **L** | LangChain | LLM, VectorDB의 조합된 파이프라인 구조, Python/JS 지원 |
| **L** | LlamaIndex | 외부 문서 인덱싱/검색 최적화 지원 (구 GPT Index) |
| **H** | Haystack | Deepset의 오픈소스 모듈식 RAG 프레임워크 |
| **R** | RAGFlow | RAG 파이프라인 구축/관리 시각적 로우코드 인터페이스 제공 |

{: .note }
> 🧠 암기법: **"L-L-H-R"** (RAG 프레임워크)
> 
...
| 암기 | 프레임워크 | 설명 |
|:-----|:----------|:-----|
| **P** | PEFT | 효율적인 파인튜닝 수행을 위한 Huggingface 라이브러리 |
| **U** | Unsloth | Llama3에 최적화된 QLoRA 기반 파인튜닝 도구 |
| **I** | InstructLab | Red Hat의 합성데이터 활용 Fine Tuning 프레임워크 |
| **O** | OpenAI API | ChatGPT 대상 파인튜닝 지원 API 도구 |

{: .note }
> 🧠 암기법: **"P-U-I-O"** (Fine-Tuning 프레임워크)
> 
> `PEFT → Unsloth → InstructLab → OpenAI API`
