...
✅ 해결: 트랜스포머로 모든 단어를 동시에 처리하자!
   → 하지만 모델이 커지면 계산량이 폭증...
   → MoE로 필요한 전문가만 골라서 효율적으로 계산!
   → 성능과 효율 두 마리 토끼를 잡음
```
...
> **트랜스포머**: Self-Attention으로 문장의 모든 단어 관계를 동시에 파악하는 AI 구조
> 
> **MoE**: 여러 전문가 중 적합한 전문가만 골라 답변하는 효율적인 AI 구조

| 개념 | 쉬운 비유 |
|:-----|:---------|
| **트랜스포머** | 교실에서 모든 학생이 동시에 서로 대화하며 정보 교환 |
| **MoE** | 병원에서 증상에 따라 해당 전문의에게만 진료받는 것 |
...
| 키워드 | 설명 |
|:------|:-----|
| **Self-Attention** | 입력 시퀀스 내 단어들의 상호 관계를 Query, Key, Value 통해 계산 |
| **병렬 처리** | RNN과 달리 순차처리 없이 GPU 병렬화 용이 |
| **희소 활성화** | 전체 Expert 중 일부만 활성화하여 연산 비용 절감 |
...
| 항목 | 설명 | 예시 |
|:-----|:-----|:-----|
| **셀**프 어텐션 | Q, K, V 연산으로 Attention Score 계산, 입력 토큰 병렬 처리 | 멀티헤드 셀프 어텐션 |
| **멀**티헤드 어텐션 | Self-Attention을 병렬로 동시 수행, 각 Head마다 다른 정보에 집중 | 8개 Head 병렬 연산 |
| **포**지셔널 인코딩 | 사인/코사인 함수로 단어 위치 정보 벡터 추가 | sin(pos/10000^(2i/d)) |
| **피**드포워드 NN | ReLU 사용, 비선형성 특징 추출, Position-Wise 완전 연결망 | FFN(x) = max(0, xW₁+b₁)W₂+b₂ |

#### 그룹 2: MoE 구성요소 `게전스로`

| 항목 | 설명 | 예시 |
|:-----|:-----|:-----|
| **게**이팅 네트워크 | 입력 분석하여 어떤 전문가 활성화할지 결정, Softmax/TOP-K Gating | Top-2 Gating |
| **전**문가 네트워크 | 여러 개의 서브 모델(FFN)로 구성, 각각 특정 유형 입력 처리 학습 | 8~64개 Expert |
| **스**파스 활성화 | 전체 전문가 중 일부만 활성화(희소 활성화)로 연산 효율성 향상 | Top-K Expert만 연산 |
| **로**스 밸런서 | 전문가 간 균형있는 분배를 위한 보조 손실(Auxiliary Loss) 추가 | Load Balancing Loss |
...
| 구분 | Transformer | MoE |
|:-----|:------------|:----|
| **연**산 방식 | **모든 레이어 사용** (Dense) | **일부 전문가만 활성화** (Sparse Expert) |
| **주**요 구성요소 | Self-Attention, Positional Encoding | Gating Network, Sparse Dispatcher |
| **활**성화 방식 | Fixed (고정된 Layer 사용) | **동적으로 Expert 선택** |
| **병**렬처리 가능성 | 제한적 (모든 Layer 종속적 동작) | Expert들이 독립적, **분산처리 가능** |
| **로**스밸런싱 | 고려하지 않음 | **로스밸런싱 고려 활용** |
| **장**점 | 학습 성능 우수, 범용성 | **연산 효율성, 확장성** |
| **단**점 | 높은 연산 비용, **계산량 증가** | 라우터 학습 복잡성, **Expert 선택 불균형** |
| **대**표 모델 | BERT, T5, GPT-3 | **Mixtral, Switch Transformer, GLaM** |
